...
use rsnano_node::{
    ...
    handles::{
        ...
        LedgerQueryHandle, LedgerStateCheckHandle, LedgerWorkThresholdHandle,
    },
...
impl RpcCommandHandler {
    pub fn new(node: Arc<Node>, enable_control: bool, tx_stop: oneshot::Sender<()>) -> Self {
        ...
        let ledger_state_checks = node.production_handles().ledger_state_checks();
        let ledger_queries = node.production_handles().ledger_queries();
