...
fn confirmed_history() {
    ...
    start_election(&node, &send1.hash());
    {
        ...
        node.consensus_services().confirming_set.set_cooldown(true);
        ...
        node.consensus_services().confirming_set.set_cooldown(false);
    }
    ...
    assert_eq!(node.ledger_query_services().ledger.confirmed_count(), 3);
}
...
fn dependent_election() {
    ...
    assert_timely_eq2(|| node.stats().get("confirmation_observer", "inactive"), 1);
    assert_eq!(node.ledger_query_services().ledger.confirmed_count(), 4);
}
