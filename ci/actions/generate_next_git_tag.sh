...
declare -a cmake_update_versions
...
if is_release_build; then
    ...
    cmake_update_versions+=("version_pre_release")
...
if [[ -n "$last_tag" ]] && echo "$tags_containing_current_commit" | grep -q "$last_tag"; then
...
if [[ $push_tag == true ]]; then
    ...
    update_cmake_lists "${cmake_update_versions[@]}"
